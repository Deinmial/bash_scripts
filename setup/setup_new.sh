...
# Запускать от root
# Перед запуском убедиться, что в SRC_DIR (ниже в коде есть переменаая) есть
# 1) ssh ключи
# 2) шрифт
# 3) openssh
# 4) klnagent64
# 5) r7-office
# Также убедиться, что настроена сеть, поставлен прокси и исключение для справок БК (127.0.0.1,localhost,::1)
# После выполнения скрипта нужно поставить криптопро, настроить screensaver, импортировать сертификаты, поставить ПО для документооборта, настроить печать
...
# ---------------------------------------------------------
# 14. Установка Справки БК
# ---------------------------------------------------------
log "Поиск и установка последней версии Справки БК..."

# Ищем все rpm пакеты SpravkiBk в директории
BK_PKG=$(find_latest_rpm "SpravkiBk-*.rpm" "$SRC_DIR")
